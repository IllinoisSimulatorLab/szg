...
if COMMAND_LINE_TARGETS != ['doc']:
    ...
    elif os.environ.get( 'SZG_USE_EXPT', 'FALSE' ) == 'TRUE':
        exptEntry = {'internal':['language','math','phleet','barrier','drivers','graphics','model','sound','interaction','framework']}
        subdirs['expt'] = exptEntry
...
if 'python_sip/src' in COMMAND_LINE_TARGETS or 'python' in COMMAND_LINE_TARGETS or 'all' in COMMAND_LINE_TARGETS:
    ...
    if os.environ.get( 'SZG_USE_EXPT', 'FALSE' ) == 'TRUE':
        dependencies = envDict['subdirs']['demo']
        dependencies['internal'].append('expt')
        szgscons.updateSzgParams( sipEnv, dependencies, externalFlags )
