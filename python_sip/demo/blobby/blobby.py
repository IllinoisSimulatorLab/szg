...
if not fw.init(sys.argv):
   raise Exception,'Unable to initialize framework.'
...
if not fw.start():
  raise Exception,'Unable to start framework.'
if not ifo.start():
  raise Exception,'Unable to start interface.'
northwall=ar_translationMatrix(0,5,-5)
ifo.setNavMatrix(northwall)
ar_setNavMatrix(ifo.getNavMatrix().inverse())
fw.loadNavMatrix()
  ...
  while 1:
    for joint in T.keys():
      ...
      while not fw.getButton(1):
        ar_setNavMatrix(ifo.getNavMatrix().inverse())
        fw.loadNavMatrix()
        dgTransform(TID[joint],T[joint]*ifo.getObjectMatrix().inverse())
        fw.setViewer()
        if fw.getButton(2):
          ifo.setObjectMatrix(arMatrix4())
          for j in T.keys():
