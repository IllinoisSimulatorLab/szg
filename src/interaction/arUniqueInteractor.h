...
/// An abstract class meant to be sub-classed by render objects that need to
/// be interacted with. It's a 'unique' interactor in that it's assumed that
/// the subject or user will only be interacting with one object at a time.
/// The class maintains a static list of members so it can keep track of who
/// should be interacted with. Who gets the 'focus' is determined by two
/// things: which objects' _interactionGroup (an unsigned int) matches the static
/// member _activeInteractionGroup (this allows for interaction with one group of
/// objects at a time), zero means an object isn't accepting interactions) and the
/// priorityScore() method (highest score for all objects within the active priority
/// group wins).  The default group is 0. Default priorityScore() returns
/// the inverse of the Euclidean distance from the wand tip. There are exceptions,
/// e.g. if an object gets 'clicked' on or some such, it can lock itself such
/// that it keeps the focus until it feels like giving it up.
///
...
class arUniqueInteractor {
  public:
    ...
    /// Static method, called by app to handle interaction with any
    /// subclass instances.
  ...
  protected:
    ...
    /// Do I currently have the focus?
    bool _touched;
    /// accepting interaction?
    bool _enabled;
    /// priority group
    unsigned int _interactionGroup;
