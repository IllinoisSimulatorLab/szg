...
bool arLogitechDriver::start() {
  ...
  if (!_eventThread.beginThread(ar_LogitechDriverEventTask, this)) {
    ar_log_error() << "arLogitechDriver failed to start event thread.\n";
    return false;
  }
  return true;
}
...
bool arLogitechDriver::_reset() {
  ...
  if (cb < 2) {
    ar_log_error() << "arLogitechDriver: _reset wrote only " << cb << " bytes.\n";
    return false;
  }
...
bool arLogitechDriver::_startStreaming() {
  ...
  if (cb < 2) {
    ar_log_error() << "arLogitechDriver: _startStreaming wrote only " << cb << " of 2 bytes.\n";
    return false;
  }
  return true;
}
...
bool arLogitechDriver::_runDiagnostics() {
  ...
  if (stat < 2) {
    ar_log_error() << "arLogitechDriver wrote " << stat << " of 2 bytes in _runDiagnostics().\n";
    return false;
  }
  const int bytesRead = _comPort.ar_read( _dataBuffer, 2 );
  if (bytesRead != 2) {
    ar_log_error() << "arLogitechDriver tracker failed to respond to the 'diagnostic' command\n"
...
//  cerr << (int)_dataBuffer[0] << ", " << (int)_dataBuffer[1] << "\n";
  ...
  ar_log_error() << "arLogitechDriver diagnostic: ";
  if (aByte & DIAG_MOTHERBOARD)
    ar_log_error() << "Motherboard failed.\n";
  if (aByte & DIAG_CPU)
    ar_log_error() << "CPU failed.\n";
  if (aByte & DIAG_ROM)
    ar_log_error() << "ROM failed.\n";
  if (aByte & DIAG_RAM)
    ar_log_error() << "RAM failed.\n";
  if (aByte & DIAG_T_FRAME)
    ar_log_error() << "T-frame failed.\n";
  if (aByte & DIAG_MOUSE)
    ar_log_error() << "Mouse failed.\n";
  if (bByte & DIAG_COMM_PORT)
    ar_log_error() << "COM port failed.\n";
  if (bByte & DIAG_EEPROM)
    ar_log_error() << "EEPROM failed.\n";
  return false;
}

bool arLogitechDriver::_update() {
  using namespace arLogitechDriverSpace;
  const unsigned numToRead = ELEMENT_SIZE - _charsInBuffer;
