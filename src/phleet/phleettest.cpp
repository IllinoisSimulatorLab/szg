...
void getTask1(void*){
  for (int i=0; i<numberGetAttr; i++){
    string result = client->getAttribute("foo", "bar1");
    if (result != "bar1"){
      cout << "Error occured in get 1 (" << result << ").\n";
    }
  }
  done1 = true;
}
...
void getTask2(void*){
  for (int i=0; i<numberGetAttr; i++){
    string result = client->getAttribute("foo", "bar2");
    if (result != "bar2"){
      cout << "Error occured in get 2 (" << result << ").\n";
    }
  }
  done2 = true;
}
...
int main(int argc, char** argv){
  ...
  client->setAttribute("foo","bar2","bar2");
    ...
    for (i=0; i<numberGetAttr; i++){
      result = client->getAttribute("foo","bar1"); 
    }
    double totalTime = ar_difftime(ar_time(), time1);
    cout << "Time to get attribute is "
